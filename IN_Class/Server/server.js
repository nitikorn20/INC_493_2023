...
app.get("/Math/:P1/:P2", (req, res) => {
    // Logic to fetch the specific resource
    let result = parseInt(req.params.P1) + parseInt(req.params.P2);
    res.json({ message: "Para1 = " + req.params.P1 + ",Para2 = " + req.params.P2, result: result });
});
...
app.post("/resources", (req, res) => {
    ...
    const sur = req.body.surname;
    // Process the received data
    console.log(
        `Received POST request with name: ${name} ,grade: ${grade}and age: ${age} and ${sur}`
    );
    // Send a response
    res.json({ message: `${name} : ${grade}` });
});
